...
struct CalendarCleanupReviewView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("Only these ordered deletions are reviewed. Confirmation reloads configuration and calendars; changed targets or order require new review. Verification covers only this local range, not global or historical retirement.").font(.footnote)
            Text("Retire legacy markers from every active publisher first. CalRelay will not edit YAML or enable ordinary sync after cleanup.").font(.footnote)
            HStack {
                ...
                if viewModel.cleanupAllowsConfirmation {
                    Button(viewModel.isLoading ? "Checking, deleting and verifying…" : "Confirm Legacy Cleanup", role: .destructive) {
