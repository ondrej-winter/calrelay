...
- Ordinary `calrelay reconcile` is dry-run by default and prints planned creates and deletes without mutating calendars.
...
- A ready ordinary dry-run or apply whose plan contains no creates or deletes returns `0` and reports that no changes are needed.
...
- Successful explanation output has two semantic sections:
  ...
  2. every planned create and delete, with the causal links and classifications required by that specification.
...
- **CLI-AC-06:** Ready no-change ordinary and cleanup runs report their respective no-change conditions and return `0` without overstating scope.
